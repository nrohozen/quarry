...
// ---------------------------------------------------------------------------
function chooseHostPattern(buckets) {
...
const SLACK_MIX = 0x190E;

function seededSlack(seed, turnIndex) {
...
const SKITTISH_MIX = 0x7B15;
const P_SKITTISH = 0.5;

function isSkittish(seed, turnIndex) {
...
function chooseHostPatternSeeded(buckets, seed, turnIndex) {
  const patterns = [...buckets.keys()];
  if (patterns.length === 1) return patterns[0];
...
function bucketize(candidates, guess) {
  const buckets = new Map();
...
// ---------------------------------------------------------------------------
function applyMove(S, adjacency, scorched, mayStay) {
  const next = new Set();
...
// ---------------------------------------------------------------------------
function letterIntel(possibleWords) {
...
// or -1 if none.
function lockedPosition(possibleWords, letter) {
  const words = Array.isArray(possibleWords) ? possibleWords : [...possibleWords];
...
// ---------------------------------------------------------------------------
function isCornered(word, adjacency, scorched) {
...
// ---------------------------------------------------------------------------
function replayGame(seed, modeConfig, guesses, wordlists) {
  const game = newGame({
...
// ---------------------------------------------------------------------------
const DAILY_EPOCH_DAY = Math.floor(Date.UTC(2026, 6, 21) / 86400000) - 1;

function dailyDayNumber(ms) {
...
const DAILY_MODE = Object.freeze({
  moveEveryNTurns: 1,
  quarryMayStay: true,
